	...
	const handleSendMessage = async (e?: React.FormEvent) => {
		...
		try {
			...
			try {
				...
				const streamPromise = (async () => {
					for await (const delta of readStreamableValue(result.newMessage)) {
						...
						if (delta !== undefined && delta !== null) {
							textContent = delta; // Use the full delta content, not concatenation
							assistantMessage = {
								role: 'assistant',
								content: textContent,
							};
				...
				console.error('Client: Stream error details:', streamError);

				// If we already have some content, don't show error
				if (textContent && textContent.trim()) {
					console.log(
						'Client: Stream error but we have content, continuing...'
					);
					return; // Don't throw, we have content
				}
